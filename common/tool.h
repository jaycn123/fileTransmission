...
static char* MD5_file(const char* path, int md5_len);
...
{
	#if defined(_WIN32)
	return MD5_file(file_name.c_str(),32);
	#else
...
static bool get_all_files(const std::string& dir_in, std::vector<std::string>& files) 
{
	...
	if ((hFile = _findfirst(p.assign(dir_in).append("\\*").c_str(), &fileinfo)) != -1)
	{
		do
		{
			...
			{
				// 保存文件的全路径
				files.push_back(p.assign(dir_in).append("\\").append(fileinfo.name));
			}
